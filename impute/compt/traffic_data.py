...
class traffic_data:
    def __init__(self, ori_data):
        ...
        self.ori_miss_rate = 1 - self.ori_W.sum() / ori_data.size
                ...
                for n2 in (1, -1):
                    for n3 in (1, -1):
                        try:
                            ...
                            pass
                if sum(neigh_info) > 0:
                    ori_data[pos1, pos2, pos3] = sum(neigh_info) / (
                        np.array(neigh_info) > 0).sum()
    ...
    def generate_miss(self, miss_ratio, miss_type="rand", miss_path=""):
        ...
        if miss_path != '' and os.path.exists(miss_path):
            ...
            return miss_data, round(true_miss_ratio, 3)
    ...
    def evaluate(self, est_data, W, precision=4):
        ...
        return round(rmse, precision), round(mape, max(precision - 2,
                                                       0)), round(
                                                           mae, precision)
